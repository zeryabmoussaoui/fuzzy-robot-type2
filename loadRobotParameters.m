...
%% VSLAM params
% TODO : apply the constant naming rules
% Note : Camera 3D world plane shall be parralal to the robot 2D ground plan
% Params for map points plan projection (for avoidance)
cameraHeight=0.30; % height in m of the lens of the camera relative to the robot plane reference
robotTotalHeight=0.5;% it shall include some safety for obstacle avoidance
vSlamLocFile='/home/sagan/ORB_SLAM2/currentPos.txt';
vSlamMapFile='/home/sagan/ORB_SLAM2/mapPoints.txt';

%% RL params
% TODO : apply the constant naming rules

% Avoidance Intrepretor param

nDig = 1; % Number of digital sensing ( in our case , the simulator the whole robot is a hit box)
nAnal = 3; %Number of analog sensing
minaSensor = 20; % distance min threshold for analog
maxaSensor = 85; % distance max threshold for analog
nSampleSens = 3; % number of sampling for analog
% nSampleSens = 10;
step=(maxaSensor-minaSensor)/(nSampleSens-1);
aSensorRange=floor(minaSensor:step:maxaSensor); % nSampleSens+1 intervals indexed from 0 to 10
nStates=(nSampleSens+1)^nAnal ;
%reward constant % TODO : delete because useless
reward_vlow =-10;
reward_low = 0;
reward_high = 10;
% Space action generation

% The action corresponds to a Rotation Correction

maxSpeed = 60 ; % in cm/s
nSampleSpeed = 1 ; %results 2*nSampleSpeed +1 Speed ( pos, neg and null)
%nSampleSpeed = 10 ;
actions=floor(-maxSpeed:maxSpeed/nSampleSpeed:maxSpeed);
nActions=length(actions);
% Qlearning Agent params

start=1; % to tell the agent an iteration starts

alpha0 = 0.6; % updateQ param , descrease on an iteration. 
eta0 = 0.5;  %  exploration/exploitation Q param , descrease on an iteration
descr = 1; % descease alpha and eps on each episode , 0.99 means we will acheive only exploitation after 1000 actions
maxAction = 1000; 
gamma = 0.5; % discount factor
% Gready params :
% alpha = 0.5 ; % still reinforce Q function
% eps = 0 ; % do only exploitation
% descr = 1 ; % no descreasing
Qtable=[];
Qtable(nStates,nActions)=0; % initialize a variable size table with a priori unknown 


% Initial State ( which state,action we came from)
s0=nStates; % no detection
a0=find(actions==0); % no speed
Q(s0,a0)=100;
save('/home/sagan/Projet_Navigation/data/Qtable.mat','Qtable')
% TODO : Cpy function used in /utilities
